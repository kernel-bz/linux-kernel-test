...
static void _xa_store_erase_find_test(struct xarray *xa)
{
    pr_fn_start_on(stack_depth);
    ...
    pr_view_on(stack_depth, "%s : 0x%lX\n", xa_to_value(entry));
#endif

    xa_debug_node_print(xa);

    //xa_debug_xarray_view(xa, 0, xa_mk_value(0));
    //xa_debug_state_view(&xas, xa_mk_value(0));
    pr_fn_end_on(stack_depth);
}
